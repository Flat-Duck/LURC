                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.operations.update", [$operation->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('service') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="service_id" id="service_id">
                                @foreach($services as $id => $service)
                                    <option value="{{ $id }}" {{ (old('service_id') ? old('service_id') : $operation->service->id ?? '') == $id ? 'selected' : '' }}>{{ $service }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('doctor') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="doctor_id" id="doctor_id">
                                @foreach($doctors as $id => $doctor)
                                    <option value="{{ $id }}" {{ (old('doctor_id') ? old('doctor_id') : $operation->doctor->id ?? '') == $id ? 'selected' : '' }}>{{ $doctor }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('patient') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="patient_id" id="patient_id">
                                @foreach($patients as $id => $patient)
                                    <option value="{{ $id }}" {{ (old('patient_id') ? old('patient_id') : $operation->patient->id ?? '') == $id ? 'selected' : '' }}>{{ $patient }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('lab') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="lab" id="lab" value="{{ old('lab', $operation->lab) }}" step="0.01">
                            @if($errors->has('lab'))
                        ...
                        <div class="form-group {{ $errors->has('price') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="price" id="price" value="{{ old('price', $operation->price) }}" step="0.01">
                            @if($errors->has('price'))
                        ...
                        <div class="form-group {{ $errors->has('notes') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="notes" id="notes">{{ old('notes', $operation->notes) }}</textarea>
                            @if($errors->has('notes'))
