                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.prescriptions.update", [$prescription->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="date" id="date" value="{{ old('date', $prescription->date) }}">
                            @if($errors->has('date'))
                        ...
                        <div class="form-group {{ $errors->has('doctor') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="doctor_id" id="doctor_id">
                                @foreach($doctors as $id => $doctor)
                                    <option value="{{ $id }}" {{ (old('doctor_id') ? old('doctor_id') : $prescription->doctor->id ?? '') == $id ? 'selected' : '' }}>{{ $doctor }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('patient') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="patient_id" id="patient_id">
                                @foreach($patients as $id => $patient)
                                    <option value="{{ $id }}" {{ (old('patient_id') ? old('patient_id') : $prescription->patient->id ?? '') == $id ? 'selected' : '' }}>{{ $patient }}</option>
                                @endforeach
                            </select>
