                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.expenses.update", [$expense->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('material') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="material_id" id="material_id">
                                @foreach($materials as $id => $material)
                                    <option value="{{ $id }}" {{ (old('material_id') ? old('material_id') : $expense->material->id ?? '') == $id ? 'selected' : '' }}>{{ $material }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('price') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="price" id="price" value="{{ old('price', $expense->price) }}" step="0.01">
                            @if($errors->has('price'))
                        ...
                        <div class="form-group {{ $errors->has('invoice_number') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="invoice_number" id="invoice_number" value="{{ old('invoice_number', $expense->invoice_number) }}">
