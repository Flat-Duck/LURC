                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.employees.update", [$employee->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $employee->name) }}">
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('gender') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\Employee::GENDER_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="gender_{{ $key }}" name="gender" value="{{ $key }}" {{ old('gender', $employee->gender) === (string) $key ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('job') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="job" id="job" value="{{ old('job', $employee->job) }}">
                            @if($errors->has('job'))
                        ...
                        <div class="form-group {{ $errors->has('salary') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="salary" id="salary" value="{{ old('salary', $employee->salary) }}" step="0.01">
