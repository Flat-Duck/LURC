                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.entries.update", [$entry->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('medicine') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="medicine_id" id="medicine_id">
                                @foreach($medicines as $id => $medicine)
                                    <option value="{{ $id }}" {{ (old('medicine_id') ? old('medicine_id') : $entry->medicine->id ?? '') == $id ? 'selected' : '' }}>{{ $medicine }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('prescription') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="prescription_id" id="prescription_id">
                                @foreach($prescriptions as $id => $prescription)
                                    <option value="{{ $id }}" {{ (old('prescription_id') ? old('prescription_id') : $entry->prescription->id ?? '') == $id ? 'selected' : '' }}>{{ $prescription }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('rx') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="rx" id="rx" value="{{ old('rx', $entry->rx) }}">
                            @if($errors->has('rx'))
